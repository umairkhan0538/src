...
class CityLearnEnv(Environment, Env):
    ...
    def get_reward(self) -> List[float]:
        ...
        #self.reward_function.kwargs['electrical_storage_soc'] = np.array([b.observations['electrical_storage_soc'] for b in self.buildings])
...
# def evaluate(self):
    ...
    def evaluate(self):
        ...
        load_factor_cost = CostFunction.load_factor(self.net_electricity_consumption)[-1]/\
            CostFunction.load_factor(self.net_electricity_consumption_without_storage)[-1]
        grid_cost = np.mean([ramping_cost, load_factor_cost])
        net_ele_consump = CostFunction.net_electricity_consumption(self.net_electricity_consumption)[-1]/\
            CostFunction.net_electricity_consumption(self.net_electricity_consumption_without_storage)[-1]
        z_net_energy = CostFunction.zero_net_energy(self.net_electricity_consumption)[-1]/\
            CostFunction.zero_net_energy(self.net_electricity_consumption_without_storage)[-1]
        
        kpis = {'Z Zero Net Energy':z_net_energy,'C Price Cost':price_cost,'G Emission Cost': emission_cost, 'R Ramping': ramping_cost, '1-L': load_factor_cost,'D Net Electricity Consumption':net_ele_consump, 'grid_cost':grid_cost}
        return kpis
